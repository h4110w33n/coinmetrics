...
EXPECTED_EXCHANGE_ASSETS = ["binance-btc", "coinbase-eth", "gemini-ltc"]
EXPECTED_ASSET_PAIRS = ["btc-usd", "eth-btc", "ltc-usdt"]
EXPECTED_INSTITUTIONS = ["grayscale"]
EXPECTED_MARKETS = ["binance-btc-usdt-spot", "coinbase-eth-usd-spot", "gemini-ltc-usd-spot"]
EXPECTED_INDEXES = ["CMBI10", "RTREE"]
EXPECTED_ASSET_ALERTS = ["time_inter_block_hi", "block_count_by_unknown_miners_6b_equals_6"]
...
class CatalogAPITests(unittest.TestCase):
    ...
    def test_get_exchange_assets(self):
        ...
        LOG.debug("\n\tTest 0: Determine if function executes without error")
        LOG.debug("\t\tFunction: get_exchanges()")
        exchange_assets = ",".join(EXPECTED_EXCHANGE_ASSETS)
        LOG.debug("\t\tArgument: exchange_assets = %s", exchange_assets)
        results = CATALOG.get_asset_exchanges(exchange_assets=exchange_assets)
        LOG.debug("\tTest 0: Pass")
        ...
        for i, exchange_asset in enumerate(EXPECTED_EXCHANGE_ASSETS):
            LOG.debug("\t\t4.%s - Searching for %s", i, exchange_asset)
            found = [ item for item in results if item["exchange_asset"] == exchange_asset]
            self.assertTrue(found)
        LOG.debug("\tTest 4: Pass")
        ...
        LOG.debug("\t\tArgument: None")
        results = CATALOG.get_asset_exchanges()
        LOG.debug("\tTest 5: Pass")
