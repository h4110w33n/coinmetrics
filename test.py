...
SAMPLE_EXPECTED_MARKETS = ['bitfinex-btc-usd-spot', 'bitstamp-btc-usd-spot', 'coinbase-eth-btc-spot', 'gemini-btc-usd-spot', 'kraken-xrp-btc-spot']
...
class BaseAPITests(unittest.TestCase):
	...
	def test_get_metrics(self):
		...
		LOG.debug("\tTest 3: Sample assets in results")
		for i,asset in enumerate(SAMPLE_EXPECTED_METRICS):
			LOG.debug("\t\t3.%s - Searching for %s", i, asset)
			self.assertIn(asset, results)
		LOG.debug("\tTest 3: Pass")
	...
	def test_get_exchanges(self):
		...
		LOG.debug("\tTest 3: Sample assets in results")
		for i,asset in enumerate(SAMPLE_EXPECTED_EXCHANGES):
			LOG.debug("\t\t3.%s - Searching for %s", i, asset)
			self.assertIn(asset, results)
		LOG.debug("\tTest 3: Pass")
...
class BaseErrorTests(unittest.TestCase):
	...
	def test_asset_metric_checker(self):
		...
		_ = CM.asset_metric_checker(ASSET, METRIC)
...
class CommunityAPITests(unittest.TestCase):
	...
	def test_get_asset_info(self):
		...
		expected_shape = {"id": str, "name": str, "metrics": list, "exchanges": list, "markets": list, "hasReferenceRate": bool, "minTime": str, "maxTime": str }
		for i,[key,value] in enumerate(expected_shape.items()):
			...
			self.assertTrue(type(result[key]) == value)
		LOG.debug("\tTest 3: PASS")
	...
	def test_get_exchange_info(self):
		...
		expected_shape = {"id": [str], "assetIdBase": [str], "assetIdQuote": [str], "minTime": [str, type(None)], "maxTime": [str, type(None)]}
		for i,[key,value] in enumerate(expected_shape.items()):
	...
	def test_get_metric_info(self):
		...
		LOG.debug("\tTest 3: Verify expected keys exist")
		expected_shape = {"category": [str], "description": [str], "id": [str], "name": [str], "subcategory": [str]}
		for i,[key,value] in enumerate(expected_shape.items()):
			LOG.debug("\t\t3.%s - Searching for %s", i, key)
			self.assertIn(key, result)
			LOG.debug("\t\t3.%s - Type check on %s", i, key)
	...
	def test_get_market_info(self):
		...
		LOG.debug("\tTest 3: Verify expected keys exist")
		expected_shape = {"assetIdBase": [str], "assetIdQuote": [str], "id": [str], "maxTime": [str, type(None)], "minTime": [str, type(None)]}
		for i,[key,value] in enumerate(expected_shape.items()):
			LOG.debug("\t\t3.%s - Searching for %s", i, key)
			self.assertIn(key, result)
			LOG.debug("\t\t3.%s - Type check on %s", i, key)
	...
	def test_get_asset_metric_data(self):
		...
		for i,[key,value] in enumerate(expected_shape.items()):
			...
			self.assertTrue(type(results[key]) == value)
		LOG.debug("\tTest 2: PASS")
		...
		self.assertTrue(type(metrics[0]) == str)
		LOG.debug("\tTest 4: PASS")
