...
class Community(Base):
	...
	def get_asset_metrics(self, asset):
		...
		"""
		if "," in asset:
			raise InvalidAssetError("Can only fetch compatible metrics from a single asset at a time.")
	...
	def asset_metric_checker(self, asset, metrics):
		...
		for metric in metrics:
			if metric in reference:
				...
				raise InvalidMetricError("Invalid metric '{}' for the given asset '{}'.".format(metric, asset))
	...
	def get_asset_metric_data(self, assets, metrics, start, end, time_agg="day"):
		...
		:return: Coin Metrics API data object. 
		:rtype: dict
		"""
